...
const statSearchGamemodeSelect = document.getElementById(
  "stat-search-gamemode",
);
...
const discordRpcEnabledCheckbox = document.getElementById(
  "discord-rpc-enabled",
);
...
toggleBtn.addEventListener("click", () => {
  ipcRenderer.send("toggle-proxy", toggleBtn.dataset.status !== "running");
});
saveApiKeyBtn.addEventListener("click", () => {
  if (apiKeyInput.value.trim())
    ipcRenderer.send("save-api-key", apiKeyInput.value.trim());
});
...
ipcRenderer.on("proxy-log", (event, message) => {
  appendToScrollbox(logOutput, message);
});
ipcRenderer.on("proxy-chat", (event, message) => {
  appendToScrollbox(chatOutput, message, true);
});
...
ipcRenderer.on("player-stats-result", (event, result) => {
  ...
  switch (result.game.apiName) {
    ...
    case "Duels":
      ...
      break;
    default:
      statsHtml += `<span>Wins:</span><span>${(d.wins || "N/A").toLocaleString()}</span>`;
  }
  statsHtml += `</div>`;

  const skinSrc = result.skinUrl
    ? result.skinUrl.replace("http://", "https://")
    : `https://crafatar.com/avatars/${result.uuid}?size=512&overlay=true`;
...
function createAliasInput(alias = "", command = "") {
  ...
  div
    .querySelector(".remove-alias-btn")
    .addEventListener("click", () => openConfirmationModal(div));
}
...
ipcRenderer.on("aliases-loaded", (event, aliases) => {
  ...
  for (const key in aliases) {
    createAliasInput(key, aliases[key]);
  }
});
...
copyLogBtn.addEventListener("click", () => {
  ...
  navigator.clipboard.writeText(logText).then(
    () => {
      showSaveFeedback(copyLogBtn, true, "Copied!");
    },
    () => {
      showSaveFeedback(copyLogBtn, false);
    },
  );
});

function showSaveFeedback(button, success, message) {
  const originalText = button.innerHTML;
  if (success) {
    button.innerHTML = `<i class="fas fa-check"></i> ${message || "Saved!"}`;
...
ipcRenderer.on("settings-saved-reply", (event, success) =>
  showSaveFeedback(saveSettingsBtn, success, "Saved!"),
);
ipcRenderer.on("aliases-saved-reply", (event, success) =>
  showSaveFeedback(saveAliasesBtn, success, "Saved!"),
);
ipcRenderer.on("api-key-saved-reply", (event, success) =>
  showSaveFeedback(saveApiKeyBtn, success, "Saved!"),
);
