...
function replaceNamesInComponent(component, nicknames) {
  ...
  const realName = Object.keys(nicknames).find(
    (name) => component.text && component.text.includes(name),
  );
  if (realName) {
    component.text = component.text.replace(
      new RegExp(realName, "g"),
      nicknames[realName],
    );
  }
...
class JagProx {
  constructor(config, env) {
    ...
    this.server = mc.createServer({
      ...
      motd:
        " ".repeat(20) +
        "§a§lJagProx §c§l[1.8-1.21]\n" +
        " ".repeat(13) +
        "§6§lHypixel Proxy §c§l- made by JagHack",
    });
    ...
    formatter.log(
      `JagProx server started on port ${this.config.port || 25565}.`,
    );
  }
  ...
  async handleLogin() {
    ...
    try {
      const authflow = new Authflow(this.client.username, cachePath, {
        authTitle: "00000000402b5328",
        flow: "live",
      });
      const response = await authflow.getMinecraftJavaToken({
        fetchProfile: true,
      });
      ...
      this.target.on("connect", () =>
        formatter.log(`Client connected to target: ${this.target.username}`),
      );
      ...
      this.client.on("packet", (data, meta) => {
        if (
          ...
          if (data.message.toLowerCase().startsWith("/play ")) {
            ...
            formatter.log(
              `Captured last play command: ${this.lastPlayCommand}`,
            );
          }
        }
        ...
        if (meta.name === "chat" && data.message) {
          try {
            ...
            if (data.position === 0 || data.position === 1) {
              console.log(
                `[JAGPROX_CHAT]${formatter.reconstructLegacyText(chatObject)}`,
              );
            }
            ...
            if (nickname) {
              if (player.displayName) {
                try {
                  ...
                  replaceNamesInComponent(component, {
      ...
      const onEndOrError = (err) => {
        ...
        formatter.log(
          `Client disconnected: ${this.client ? this.client.username : "Unknown"}`,
        );
      ...
      formatter.log(`Authentication error: ${e.message}`);
      if (this.client) {
        this.client.end("Authentication failed. Please try again.");
      }
    }
  }
